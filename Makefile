...
#
# The eval lanes are prerequisites because `go test ./...` cannot reach them:
# every eval file carries a build tag, so the untagged test step below compiles
# none of it (iss-2608311632382737). Without them a defect in the evals passes
# every local gate and surfaces only in CI, where the job that runs them is not
# a required status check — the eval that certifies the cold-reading firewall
# was guarded by nothing that could stop anything. Both lanes are named even
# though `smoke` compiles a superset of `evals-cold-reading`'s files: they are
# separate tag sets, so a cold-reading file reaching for a smoke-only helper
# compiles under one and not the other, which is the split CI's two jobs cover.
# About five seconds each on a warm cache, against roughly a minute for the
# gates already here.
