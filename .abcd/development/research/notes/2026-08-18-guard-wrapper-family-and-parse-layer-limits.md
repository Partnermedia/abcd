...
`internal/core/guard/match.go:13` steps past "wrappers" to find the real command.
...
- **[Claude Code][cc-permissions]** states that argument-constraining Bash patterns are "fragile",
  ...
  so pair it with one of the options above", and that [sandbox restrictions][cc-sandboxing] hold
  "even if a prompt injection bypasses Claude's decision-making". It *refuses to
...
readable vendored snapshot held 288 binaries, 182 `shell` and 106 `command`.
...
and warn if any suffix is a hazard. `payload.go:14` already records why this
...
on both planes (`cli.go:933`, `guard.go:19`); extrapolating the `env` case gives
...
|---|---|
| `nice`, `nice -n 5`, `setsid`, `flock`, `stdbuf` | warn |
| `busybox sh -c`, `su -c`, `runuser -c`, `script -c` | **still silent** |

Running each speculative suffix through `expandPayloads` recovers `busybox sh -c`
at **zero** additional false positives. The other three need the exec-string
table below.
...
hazard-shaped text under an unrecognised argv[0]** — `rg git push --force`,
`grep -rn gh repo delete .`. Quoting silences it. A `git`/`gh`/`rm` argv[0]
silences it, which makes `git grep git push --force` quiet while `rg …` warns —
an arbitrary asymmetry worth documenting. This repo's subject *is* the hazard
registry, so expect the warn interactively even though it never fired on a
committed line.
...
shipped known-good gate**:
    ...
    --- FAIL: TestBundledEntriesPassAdmissionGate/git-clean
        ...
        (verdict "warn"); the true-negative floor is 100%
...
The plan's first draft had four errors here. Recorded with the verification so
...
| `nsenter` | help renders `-S/--setuid` as optional; it is **required_argument**. Value flags are `-t -W -S -G` only. | `nsenter -S /bin/echo hi` → `failed to parse uid: '/bin/echo'`; `nsenter -m /bin/echo hi` → took no value |
...
**`-S` is required-arg in `unshare` and optional-arg in `nsenter` — same package,
same version, same letter.** That single fact is the strongest argument that a
...
`sudo.ws`, `cursor.com`, `man7.org` and others; claims sourced through those are
second-hand and are marked as such in the working record. Read as primary:
Claude Code permissions and sandboxing docs, the GTFOBins README and
`_data/functions.yml`, the Cursor security advisory, sudoers(5) via Ubuntu
manpages, the OWASP OS Command Injection Defense Cheat Sheet, and the openai/codex
`execpolicy` README.
