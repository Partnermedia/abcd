...
- `itd-198` — an assembly reports what it would cost before a reading is commissioned: bytes and an estimated token count per material kind, a `test` kind split from `source`, and the kind recorded per manifest item so the report is checkable against the attestation (carries iss-2608311501186646; the number is cheap where the fix is a redesign)
