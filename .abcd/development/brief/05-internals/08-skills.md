...
- A **brief surface file** (`04-surfaces/NN-<verb>.md`) describing acceptance criteria, interaction flow, and side effects (or a sub-verb row in an existing parent's surface file) — for example `docs`, `history`, and `version` are chapters `10`–`12`. The CLI-only binary verbs `changelog`, `rules`, and `spec` have a Go verb but no surface chapter of their own; the surfaces index lists them under [§ Operator-internal verbs](../04-surfaces/README.md#operator-internal-verbs) with the record that delivered each.
