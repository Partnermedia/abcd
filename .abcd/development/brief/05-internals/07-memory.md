...
| `/abcd:memory ingest <path-or-https-url>` | Read external source, distil into typed pages with citation, append to log. A remote source must be https (admission and every redirect hop), the posture of `abcd update`. Default: do NOT store original. Flag: `--keep-original` (opt-in storage; the lifeboat licence-gate allowlist is required before a kept original could ever publish — via `/abcd:disembark`, not launch; adr-28). |
