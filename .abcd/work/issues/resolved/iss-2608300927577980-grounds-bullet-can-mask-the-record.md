...
impact: internal
grounds: "pursued: we expect a write that verifies its own result to be the only durable guard here, because banning the pattern that happens to break the mask today would miss every future one while a read-back check catches them all"
---
