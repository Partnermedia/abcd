...
resolution: "docs lint resolves its config from the working directory (.abcd/docs-lint.json); the plugin-root dependency in internal/core/lint is gone, so the detector condition is unreachable"
impact: internal
---
