...
resolution: "Already covered by the bundled rm-rf-after-cd-chain entry: the incident shape is known-bad fixture #1 and the incident capture that names it is known-good #1, pinned by TestIncidentCaptureIsKnownGoodFixtureOne. No code change; the three-step form the incident actually took (mkdir, cd, rm -rf * in one chain) was added to the corpus so the coverage is proven rather than asserted, in both the && and ; separator forms."
impact: internal
---
