...
The fix is doc-side because the alternative is unwiring work, not doc repair. An exclusion mechanism does exist: `internal/core/launch/bundle.go:26-31` denies the `.abcd` namespace structurally and `bundle_test.go` pins it. It is simply not on the path any release takes. The wired `abcd launch ship` exercises the deny through `launch.PrecheckPayload` and `launch.RenderPayload`, which resolve the bundle (`internal/core/launch/render.go:192`); `launch.Ship`, the would-publish stub that halts at `WouldPublish` with no network call (`internal/core/launch/ship.go:36-41`), has no production caller; and `release.yml` invokes neither, uploading the binaries directly. Making the blanket claim true therefore means wiring that path and finding an answer for the two channels it does not govern (a marketplace install of the repository root, and GitHub's auto-generated source archives). Out of scope here.
