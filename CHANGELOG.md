...
- **The guard's own limits are on the record, and so are the sources that
  ...
  execution-layer control behind it. Nothing in the guard's behaviour changes
  yet; what lands here is the decision, the evidence behind it, and credit.
  `ACKNOWLEDGEMENTS.md` gains five § Inspirations entries for the precedents that
  shaped it — the permission and sandboxing models of two agent harnesses, a
  third's sandbox/approval split, GTFOBins' shell/command taxonomy, and sudo's
  `NOEXEC` — with the full source list registered in the design record. (adr-42,
  iss-272)
...
- **The attribution gate can be written about again.** Its banned-footer and co-authorship rules are anchored to line start so prose quoting them mid-sentence stays documentation — but a fenced code block, the natural way to show a literal shape, was refused exactly as a real footer would be, and the change that tightened the rule tripped over this in its own pull-request body. A fenced block is quoted material rather than the document's own voice, so it is now removed before any check runs. That reads both ways: a banned form inside a fence is an example, and a trailer inside a fence is an example too — it no longer counts as the disclosure. **The pull-request body only:** the whole justification is that a forge *renders* a fence as a code block, and a commit message is never rendered — `git log` shows it verbatim — so the commit check keeps the unstripped text and the mid-sentence convention. **The stripped region stays inside what markdown renders as code**, which is what keeps the concession safe: a line the gate skips but the forge renders as an ordinary paragraph would be a real footer, silently passed. So the matcher pairs fences the way CommonMark does — a block closes only on the same marker character, at least as long, with nothing after it, and a backtick opener carrying a backtick in its info string is an inline code span rather than a fence. An unterminated block strips nothing, which both fails closed and lets a longer fence quote a shorter one, the shape needed to document this rule at all. A fence indented four spaces or led by a tab is not a fence and stays checked; one nested inside a list item is indented past the limit and is not stripped either (iss-270). Permitting a fenced footer in a body costs little against the actual threat — this gate defends against a footer a *tool appends by default*, and no tool we have met fences its own; it was never a defence against a determined author, who would simply delete the footer. (iss-268)
- **The attribution gate can be written about again.** Its banned-footer and co-authorship rules are anchored to line start so prose quoting them mid-sentence stays documentation — but a fenced code block, the natural way to show a literal shape, was refused exactly as a real footer would be, and the change that tightened the rule tripped over this in its own pull-request body. A fenced block is quoted material rather than the document's own voice, so it is now removed before any check runs. That reads both ways: a banned form inside a fence is an example, and a trailer inside a fence is an example too — it no longer counts as the disclosure. **The pull-request body only:** the justification is that a forge *renders* a fence as a code block, and a commit message is never rendered — `git log` shows it verbatim — so the commit check keeps the unstripped text and the mid-sentence convention. **What this defends is footer-last.** Bodies picked up a tool's default footer across 78 pull requests — an accident at scale, not an adversary — and a tool appends its footer last, which is caught whatever precedes it. A body *constructed around* the footer is not defended, and never was: a plainly fenced footer passes by design. Within a document-level context the matcher pairs fences the way CommonMark does — same marker character, at least as long, nothing after it, and a backtick opener carrying a backtick in its info string is an inline code span rather than a fence — because a looser test let a footer be stripped while the forge rendered it as an ordinary paragraph. An unterminated block strips nothing, which both fails closed and lets a longer fence quote a shorter one, the shape needed to document this rule at all. **A document containing an HTML block strips nothing**, since the matcher cannot model one and striking less can only over-reject. A fence indented four spaces or led by a tab is not a fence; one inside a first-level list item is *under* the indent limit and is stripped although the forge renders it visibly, which is the known residual (iss-270). Bad invocations exit 2 again, as documented. (iss-262 follow-up, iss-268)
