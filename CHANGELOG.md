...
- **The developer docs describe the gate suite that actually runs** (iss-37).
  ...
  build, vet, test and the race-enabled internal tests, and attribute the format
  gate to CI's own `gofmt` step rather than to preflight; the `ci.yml` step
  comment states that the record-lint step blocks.
