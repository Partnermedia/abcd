...
- **`abcd guard` no longer misses a force-push behind an unlisted git global option.** The six git entries listed the global options that consume the following token so the matcher could step over them and find the subcommand — but the list stopped at seven and git has nine. `--attr-source <tree>` (git 2.40+) and `--config-env <name>=<var>` were absent, so their value was read as the subcommand: `git --attr-source HEAD push --force origin main` performs the identical forced update and was silently **allowed**, as were `--no-verify` commits and `reset --hard`. Every git blocker was bypassable by the same prefix, and only the separate-token spelling evaded — the glued `--opt=value` form was already skipped as an ordinary flag. The enumeration is now complete against git 2.43's own option handling, and a test asserts **completeness** against the registry rather than re-checking the two spellings that happened to be missing, so a list that drifts again fails the build instead of a session. (gh-299)
