...
- **Branch + PR** for substantive changes; CI (build, `gofmt -l .`, vet, test and
  race-enabled internal tests on macOS + Linux, plus record-lint, docs-lint,
  gitleaks and zizmor) gates the merge. `make preflight` runs the build, vet, test
  and race steps locally via the pre-push hook, together with the lint-reviews,
  record-lint and docs-lint gates; the format gate is CI's own step, so run
  `gofmt -l .` before pushing.
